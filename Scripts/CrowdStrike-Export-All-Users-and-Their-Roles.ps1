  ...
  Param ([Parameter(Mandatory=$true)][string]$csvOutDir)
    ...
    $fusers | ForEach-Object {
        ...
        $fuserInfo =[pscustomobject]@{
            ...
            'Email' = $_.uid
            'UUID' = $_.uuid
            'Falcon Roles' = ($fuserroles.display_name -join ", ")
...
User2Roles -csvOutDir $sLogPath

    Write-Host "Exported Falcon users and roles successfully." -ForegroundColor Green
    # Log that a token was received
    Log-Write -LogPath $sLogFile -LineValue "Exported Falcon users and roles successfully."
    # Finalize the log
    Log-Finish -LogPath $sLogFile
    Break
    ...
    # Log that a token was received
    Log-Error -LogPath $sLogFile -ErrorDesc "Export of Falcon users and roles was NOT successful. Exiting." -ExitGracefully $True
    Break
